...
产物：`linear/sgd_batch_size.png`、`svm/svm_linear_loss.png`。MacBook Air 端到端约 3 分钟。
...
**L1（稀疏）**：无关的 6 维会被精确压到 0；$\lambda = 1$ 时只剩 $x_1, x_3^2$；$\lambda = 5$ 全灭。
...
**三个重要自变量**：$x_1,\ \cos(x_2),\ x_3^2$，与生成机制完全一致。

**现象解释**：L2 的梯度是 $2\lambda \theta$，在每一维上是连续的线性收缩，不会让某一维在有限迭代中达到恰好 0；L1 的次梯度 $\lambda \operatorname{sgn}(\theta)$ 在 $\theta_j = 0$ 处的次微分是整个区间 $[-\lambda, \lambda]$，只要数据项梯度绝对值不超过 $\lambda$，$\theta_j = 0$ 就是 KKT 点。因此 L1 天然具备稀疏选择能力，L2 只能做各向同性收缩。
...
`grad_checker` 通过中心差分 $\frac{J(\theta + \varepsilon e_j) - J(\theta - \varepsilon e_j)}{2\varepsilon}$ 逐维近似梯度，我在 $\theta = 0$ 与随机 $\theta$ 上都通过了 `tolerance = 1e-4`。
...
用 `np.random.RandomState(0).randint` 均匀采样 `batch_size` 个下标；按 2.6.1 公式更新。作业要求验证集记录**不含正则项**的纯 MSE：`validation_hist[i] = (X_val @ theta - y_val)^2 / n_val`。

#### 2.6.3 批大小实验

小 batch 梯度方差大，$\operatorname{Var}(\hat g) \propto 1/n$，要稳定收敛必须同时调小步长。以下是我为每个 $n$ 选的步长和跑 3000 步后的 val MSE：
...
3. **稳定性**：$\nabla_w L = w - \sum_i \mu_i y_i x_i = 0$，即 $w = \sum_i \mu_i y_i x_i$；$\nabla_b L = -\sum_i \mu_i y_i = 0$；
4. **互补松弛**：$\mu_i \left[ y_i(w^\top x_i + b) - 1 \right] = 0,\ \forall i$。
...
对称性由 $\Phi(x)^\top \Phi(x') = \Phi(x')^\top \Phi(x)$ 显然。$\blacksquare$
...
直接在 3.3.2 的对偶中把内积 $x_i^\top x_j$ 替换为 $k(x_i, x_j)$（等价于在 RKHS 中跑原问题）：
