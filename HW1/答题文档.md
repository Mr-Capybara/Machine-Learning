...
助教在 2.2.2 只让 $(x_1, \cos x_2, x_3^2)$ 与 $y_1$ 有关，所以 L1 能干净地把无关的 6 维压到 0。自己构造了一个 9 项全部非零的 $y_1$：
...
2. **$\lambda$ 变大**（λ=1e-2），$\cos x_1$ 从 +0.57/+1.03 直接跳到接近 0 甚至 −0.03 变号，$x_1^2$ 也被压到 0.3 上下
3. **$\lambda$ 再大**（λ=1），两种正则都把所有系数压到 0，模型退化为只拟合 bias，val MSE 涨到 7
...
$y_2$ 上 cosine + $k=10$ 最好（0.80），$y_3$ 上各度量差距不大、最好 0.38。接下来以此为基础扫描数据量。
...
1. **数据量越大，KNN 在回归任务上越精细**。$y_2$ 上固定 $k=10$ 时，test MSE 从 $n=50$ 的 1.391 降到 $n=1000$ 的 0.803，下降了约 42%。$k=5$、$k=30$ 同样呈单调下降趋势。这符合 KNN 作为非参数估计的理论性质：预测值是局部邻域的平均，$n$ 越大则每个查询点的 $k$ 个近邻在特征空间上越接近它本身，偏差越小。

2. **最优 $k$ 随 $n$ 增大而增大**。$n=50$ 时最好的 $k$ 是 5（MSE=1.334），$k=10$ 反而略差；$n \ge 300$ 之后 $k=10$ 就稳定超过 $k=5$；$n=1000$ 时两者几乎持平。原因在于 $k$ 控制的是"偏差—方差"的折中：训练样本少时，只能取较小的 $k$ 保证邻居真的"邻近"（降偏差），但代价是方差大；数据变多后，稍大的 $k$ 才能在不引入过多偏差的前提下充分平滑掉噪声。

3. **$k=1$ 对数据量的改善最不敏感**。$n$ 从 50 增到 1000，$k=1$ 的 MSE 只从 2.388 降到 1.707，降幅约 29%，远小于 $k=5 \sim 30$ 的降幅。因为 $k=1$ 的预测完全由最近的那一条训练样本决定，其方差来自标签本身的噪声 $\varepsilon$，增加样本数只能让"最近邻"更近一点，却压不住单样本噪声。这与课上 KNN 的渐近分析是一致的：$k=1$ 的贝叶斯风险比是 2 倍，而 $k \to \infty$、$k/n \to 0$ 才能达到贝叶斯最优。

4. **$y_3$ 上增加数据量几乎没有改善**，6 个 $n$ 值对应的准确率在 0.32~0.36 之间来回抖动，没有明显趋势。这不是 KNN 本身的问题，而是话题 4 推出的贝叶斯错误率上界 0.486 已经锁死了所有模型的表现：$n=1000$ 时模型已经接近这个天花板，再增加数据也只能逼近 0.486，而 100 条测试样本的抽样波动就足以盖过这点提升。

综上，**当任务本身的贝叶斯错误率较低时（如 $y_2$），KNN 的精度随 $n$ 稳步提升，且最优 $k$ 会随之增大；当贝叶斯错误率接近随机猜测时（如 $y_3$），增加数据量带来的收益十分有限**。另外还要注意，即便 $y_2$ 上 $n=1000$ 时最好的 KNN（0.80）也远差于线性回归在同一任务上的 test MSE 0.042。这是因为 $y_2$ 的真实生成机制是 35 维线性加噪，线性模型只需估计 35 个参数即可，而 KNN 要用局部平均逼近同样的线性函数，在 35 维空间中需要的样本数远不止 1000。
...
- 本次作业未与任何同学交流讨论。
- 本次作业使用了大模型（Claude）辅助调试 `kernel_svm_subgrad_descent` 的数值不稳定问题，据此学习并采用了 Pegasos-kernel 稀疏更新；格式排版亦由大模型辅助。所有公式推导、代码逻辑、实验结果均由本人独立完成并在本机验证。
