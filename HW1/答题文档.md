...
- $\alpha = 0.2$ 这一行全部跑崩成 NaN，步长显然是超过了二次函数收敛阈值；
- $\lambda$ 从 0 到 $10^{-4}$ 差别微乎其微，我一开始以为是 bug，后来想到 y2 的噪声本身就小（`noise_std=0.2`），模型欠拟合不严重，加不加正则都一样；
- $\lambda$ 提到 $0.1$ val MSE 直接冲到 1，说明正则太猛，把信号一起压住了，这个"正则不是越大越好"的手感和 2.2.2 的 L1/L2 实验是一致的。
...
这题我选了话题 2、3、4 三个都跑了，分别对应"正则化恢复真实系数""KNN 打不过线性模型""Softmax 准确率的天花板"。实验脚本放在 `experiments/` 下（`topic2.py / topic3_knn.py / topic4_bayes.py`）。

#### 话题 2：9 个基函数都与 $y_1$ 有关时，正则化能不能"学回真实系数"

助教在 2.2.2 只让 $(x_1, \cos x_2, x_3^2)$ 与 $y_1$ 有关，所以 L1 能干净地把无关的 6 维压到 0。我自己构造了一个 9 项全部非零的 $y_1$：
...
| ------------ | --------- | ------- | ----- | ----------- | ------- | ---------- | ------- |
| 真实（同向） | —         | —       | +     | +           | +       | −          | +       |
| L2           | 1e-4      | 0.21    | +5.32 | **+1.05**   | +3.02   | −0.55      | +2.03   |
| L2           | 1e-2      | 0.42    | +3.52 | **−0.03**   | +0.27   | −0.43      | +0.97   |
| L2           | 1         | 7.06    | +0.09 | +0.0002     | +0.001  | +0.07      | +0.11   |
| L1           | 1e-4      | 0.25    | +5.20 | **+0.70**   | +2.23   | −0.51      | +1.74   |
| L1           | 1e-2      | 0.26    | +4.93 | **−0.0002** | +0.32   | −0.45      | +1.51   |
| L1           | 1         | 7.86    | ≈0    | ≈0          | ≈0      | ≈0         | ≈0      |

有几件事很有意思：

1. **$\lambda$ 很小时 val MSE 最低**（L2 λ=1e-4 时 0.21），但这时学到的 $\cos x_1$ 系数是 **+1.05**，是真实值 +0.5 的**两倍多**。模型为了压低训练误差，在多个强相关的基函数之间"东挪西借"，学出的系数和真实系数差得离谱；
2. **$\lambda$ 变大**（L2 λ=1e-2、L1 λ=1e-2），$\cos x_1$ 的系数就从 +1.05 直接跳到 −0.03，甚至**符号都翻了**。这个翻转在 L1 下更明显：λ=1e-2 时 $\cos x_1$ 和 $\cos x_3$ 都被砍成 0，只留下 $\cos x_2$；
3. **$\lambda$ 再大**（λ=1），两种正则都把所有系数压到 0，模型退化为只拟合 bias，val MSE 暴涨到 7。

**为什么会这样？** $x_1 \in [-3, 3]$ 时 $x_1,\ x_1^2,\ \cos x_1$ 在归一化后高度相关（$x_1^2$ 对 $x_1$ 的 $R^2 \approx 0.5$，$\cos x_1$ 对 $x_1^2$ 也有明显关系）。多重共线性下，最小二乘解不唯一，"真实系数"和"别的组合"在训练集上几乎等价；正则化只是在众多等效解里选**范数最小的那个**，不保证是**真实的那个**。所以正则化能改善**预测误差**，但不保证恢复**模型结构**——这也是 2.2.2 里助教特意挑"只有一个非零项"的设计原因，不然 L1 的稀疏性看起来就不那么漂亮了。

#### 话题 3：用 KNN 拟合 $y_2$ 和 $y_3$

我用 `sklearn.neighbors` 直接调包。特征按训练集 min/max 归一化后，对比不同 $K$、不同距离度量、不同训练数据量。

**$y_2$（回归）test MSE**：

| metric     | k=1  | k=3  | k=5  | k=10     | k=30 | k=100 |
| ---------- | ---- | ---- | ---- | -------- | ---- | ----- |
| euclidean  | 1.67 | 1.01 | 0.93 | 0.87     | 1.03 | 1.19  |
| manhattan  | 2.03 | 1.14 | 1.03 | 0.93     | 0.97 | 1.24  |
| **cosine** | 1.71 | 0.88 | 0.81 | **0.80** | 0.98 | 1.17  |

线性 GD 在 $y_2$ 上的 test MSE 是 **0.042**（2.5 的结果），KNN 最好的 0.80 比它差了整整**20 倍**。原因很直观：$y_2$ 的生成是 35 维纯线性加噪，本来就是线性模型的主场；KNN 在 35 维空间里找"邻居"，维度灾难下的距离早就近似同阶，再加上 $y_2$ 的系数量级不一（比如 $x_{11}$ 的系数是 0.0013、$x_1$ 是 0.12），归一化后距离权重混乱，邻居选得并不可靠。

另外 U 形曲线很明显：k=1 过拟合到单个样本（噪声都学进来），k=100 欠拟合（几乎在对训练集求均值），k=10 是折中。
...
| manhattan | 0.33 | 0.32 | 0.34 | 0.33 | 0.27 | **0.38** |
| cosine    | 0.32 | 0.30 | 0.34 | 0.33 | 0.31 | **0.38** |

$y_3$ 上 KNN 最好只到 0.38，和 Softmax 的 **0.39** 几乎一模一样。这不是巧合——看话题 4 就知道，0.48 左右就是这个数据集的**理论上限**，任何模型都不可能显著超过它。而且 k=100 时所有度量都给出 0.38，基本等于**猜多数类**，可以对照下一段的类别边缘分布（`[0.245, 0.259, 0.257, 0.238]`，最多类占 25.9%，加上测试集抽样波动就是 0.38）。

**数据量影响**（k=10, euclidean, $y_2$）：

| n_train  | 50   | 100  | 300  | 500  | 800  | 1000 |
| -------- | ---- | ---- | ---- | ---- | ---- | ---- |
| test MSE | 1.40 | 1.03 | 1.03 | 1.06 | 0.95 | 0.87 |

从 50 到 1000，test MSE 从 1.40 降到 0.87，但始终远远追不上线性模型。单调下降也很符合 KNN "**非参数模型吃数据**"的本质——想再大幅提升，估计得把训练集扩到万级别。

#### 话题 4：Softmax 准确率 0.39 是因为数据本身有贝叶斯错误率

2.7 测出来 test acc = 0.39，一开始我也怀疑是模型问题。但仔细看 `generate_data.py` 的 y3 流程：每类先算线性得分 $s_c$，加 `noise_std=0.1` 的高斯噪声，减一个固定偏移 $[0.864, 10.548, -0.463, 0.729]$，再过一个 `temperature=2` 的 softmax 得到概率，**按概率抽样而不是取 argmax**：

$$
p_c(x) = \frac{\exp(s_c(x) / 2)}{\sum_{c'} \exp(s_{c'}(x) / 2)}, \qquad y \sim \operatorname{Cat}(p(x))
$$

这是关键：**标签本身就是随机的**。最优的贝叶斯分类器只能预测 $\hat y = \arg\max_c p_c(x)$，它期望的准确率上界是

$$
\operatorname{Acc}^* = \mathbb{E}_x \!\left[\, \max_c p_c(x) \,\right]
$$

我写了 `topic4_bayes.py`，按 `generate_data.py` 原样重放 100k 个样本，直接算这个期望：
...
所以 **0.486 就是天花板**。实测测试集 acc = 0.39，离上界差约 9.6 个百分点——这部分是 100 条测试样本的抽样波动 + 模型对 p(x) 的估计误差，完全合理。话题 3 里 KNN 在 y3 上也打不到 0.39 以上，是同一个天花板挡住的。

再看偏移向量：第 2 类被减 10.548，softmax 后 $\exp(-5.3)$ 比其它类小 5 个量级，所以**几乎采不到第 2 类**。但从上面的边缘分布 `[0.245, 0.259, 0.257, 0.238]` 看，四类竟然几乎均衡——这说明助教在设计偏移量时是经过校准的（第 2 类 bias 本身就极大，减掉后刚好拉平）。

**结论**：0.39 这个数字不是模型烂，是数据设计的必然上界。要想破 0.5，除非调低 `temperature`、缩小 `noise_std`、或换一套 y3 的生成机制——换模型没用。
...
按 3.3.3 推的次梯度直接写，每步随机抽 batch：

$$
\hat g = \lambda w - \frac{1}{|B|} \sum_{i \in B,\ y_i w^\top x_i < 1} y_i x_i,\qquad w \leftarrow w - \alpha\, \hat g
$$

超参随便调了几组，最后用 $\alpha = 0.05$、$\lambda = 10^{-4}$、`num_iter = 20000`、`batch = 32`。损失曲线在 `svm/svm_linear_loss.png`，大约 5000 步以后就比较平稳了。
...
核方法直接在对偶坐标 $\theta \in \mathbb{R}^m$ 下跑（$w = \sum_j \theta_j \Phi(x_j)$）。我一开始写的版本是直接稠密更新 $\theta \leftarrow \theta - \alpha(\lambda K\theta - \dots)$，但每步都要 $O(m^2)$ 的矩阵乘，3260 条样本跑 4000 步要 10 分钟以上。

后来查了一下 Pegasos 这个经典算法，发现它的更新可以拆成两部分：

- 正则项对应 $\theta$ 的**整体收缩**：$\theta \leftarrow (1 - \eta_t \lambda) \theta$；
- hinge 项对应**只改 batch 内违反 margin 的那几个坐标**：$\theta_i \mathrel{+}= \frac{\eta_t}{|B|} y_i$。

步长用 Pegasos 推荐的 $\eta_t = 1/(\lambda t)$。这样每步只有 $O(|B| \cdot m)$ 算 batch 上的 $K[B] \theta$，4000 步大概半秒跑完，比我原来的版本快了差不多一个数量级。

预测就是 $f(x) = \operatorname{sign} \sum_j \theta_j\, k(x_j, x)$。

我扫了 6 组超参，顺便对比了一下线性核和 RBF 核：
...
我本来以为上 RBF 核会有明显提升，结果最好的 RBF（$\gamma = 0.1$）也就比线性核（0.8568）高 0.6 个百分点，而且对 $\gamma$ 特别敏感：

- $\gamma$ 太小（0.001）：$k(x, x') \to 1$，核矩阵接近全 1 阵，模型退化成常数预测，F1 直接 0（全判正类）；
- $\gamma$ 太大（0.5 以上）：$k(x, x') \to \delta_{xx'}$，核矩阵接近单位阵，训练集完美拟合、验证集泡汤；
- 只有在 $\gamma \approx 0.05 \sim 0.1$ 这一小段区间才 work。

我的理解是：TF-IDF 特征本身就 5000 维、极度稀疏，高维空间里样本两两之间几乎都是"远距离"，RBF 的 $\exp(-\gamma \|x - x'\|^2)$ 要么把所有距离都压成 0、要么都压成 1，很难拉开区分度。线性核反而刚刚好，因为在这么高的维度里文本本身就接近线性可分。如果换成低维稠密的 embedding（比如把每条文本用 Word2Vec 平均成 300 维），可能核方法的优势才会显现出来。

#### 3.5.4 最终模型

挑 val_acc 最高的那个：Kernel SVM，RBF 核，$\gamma = 0.1$，$\lambda = 10^{-4}$。
...
joy 的召回 622/707 = 88.0%，sadness 的识别 571/676 = 84.5%，两类也还算均衡。
