...
1. **$\lambda$ 很小时 val MSE 最低**（L2 λ=1e-4 时 0.21），但这时学到的 $\cos x_1$ 系数是 **+1.05**，是真实值 +0.5 的两倍多
2. **$\lambda$ 变大**（L2 λ=1e-2、L1 λ=1e-2），$\cos x_1$ 的系数从 +1.05 跳到 −0.03，甚至变号
3. **$\lambda$ 再大**（λ=1），两种正则都把所有系数变成 0，模型退化为只拟合 bias，val MSE 涨到 7
...
用 `sklearn.neighbors` 直接调包。特征按训练集 min/max 归一化后，对比不同 $K$、不同距离度量、不同训练数据量。
